pipeline {
    ...
    stages {
        ...
        stage('Packaging/Pushing imagae') {
            ...
            steps {
                withDockerRegistry(credentialsId: 'dockerhub', url: 'https://index.docker.io/v1/') {
                    sh 'docker build -t darknguyen102/springboot .'
                    sh 'docker push darknguyen102/springboot'
                }
            }
        }
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                echo 'Deploying and cleaning'
                sh 'docker image pull darknguyen102/springboot'
                sh 'docker container stop khalid-springboot || echo "this container does not exist" '
                sh 'docker network create dev || echo "this network exists"'
                sh 'echo y | docker container prune '

                sh 'docker container run -d --rm --name khalid-springboot -p 8081:8080 --network dev darknguyen102/springboot'
            }
        }
